...
var $MODEL_TRAIN_SCORE; //모델 학습정보의 모델성능평가
var $MODEL_TEST_SCORE;
var projectSequencePk;
var selectedModelPk; //모델 테스트결과의 score점수
...
var fnSetModelInfo = function (model) {
  ...
  html += "<tr><th>모델 성능 평가</th><td colspan='2'>" + model.validation_summary.value.holdout_score.toFixed(3) + "</td></tr>"; //트레인 전역변수에 세팅
...
var fnDeleteModel = function () {
  if (confirm("모델을 삭제하시면 배치관련 데이터도 같이 삭제됩니다. \n삭제하시겠습니까?")) {
    var response = fnDeleteModelByAjax(projectSequencePk, selectedModelPk);
...
var fnStopModel = function () {
  if (confirm("학습을 중지 요청을 하시겠습니까?")) {
    ...
    var response = fnStopAndRestartModelByAjax(projectSequencePk, selectedModelPk, data);
...
var fnModelTest = function (popup) {
  ...
  var response = fnModelTestByAjax(projectSequencePk, selectedModelPk, data);
...
var showScore = function (data) {
  var html = "";
  html += "<p class='fontSize_20 fontWeight_bold'>TRAIN SCORE: " + $MODEL_TRAIN_SCORE + "</p>";
  html += "<p class='fontSize_20 fontWeight_bold'>TEST SCORE: " + data.score + "</p>";
  return html;
}
...
var fnModelTestResultChart = function () {
  am4core.ready(function () {
    ...
    var data1 = {
      "범주값": "학습(Train)",
      "frequency": $MODEL_TRAIN_SCORE
    };
    var data2 = {
      "범주값": "검증(Test)",
      "frequency": $MODEL_TEST_SCORE
    };
    list.push(data1);
    list.push(data2);
