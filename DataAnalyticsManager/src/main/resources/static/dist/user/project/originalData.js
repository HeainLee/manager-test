...
var projectSequencePk, selectedInstancePk, selectedOriginalDataPk, selectedPreprocessedDataPk, selectedModelPk;
var preprocessTestFlag = false;
...
var fnInit = function () {
  projectSequencePk = $("#projectSequencePk").val(); // 프로젝트 정보를 가져온다.

  fnGetProjectInfo(projectSequencePk); // 원본 리스트 가져오기

  fnGetOriginalDataList(projectSequencePk, function () {
    pageInit();
  });
...
var fnGetOriginalDataList = function (projectSequencePk) {
  ...
  for (var i in originalDataList) {
    ...
    if (i == 0) {
      ...
      selectedOriginalDataPk = oData.id;
...
var fnDeleteOriginalData = function () {
  if (confirm("원본 데이터를 삭제하면 생성된 전처리, 모델이 같이 삭제됩니다. \n삭제하시겠습니까?")) {
    var response = fnDeleteOriginalDataByAjax(projectSequencePk, selectedOriginalDataPk);
...
var fnCreateOriginalData = function () {
  ...
  if (fnNotNullAndEmpty(localFile)) {
    if (confirm("생성하시겠습니까?")) {
      ...
      var response = fnCreateOriginalDataByAjax(projectSequencePk, data);
...
function getPossiblePreProcess() {
  ...
  $.ajax({
    type: "post",
    data: {
      "projectSequencePk": projectSequencePk,
      "originalDataSequencePk": selectedOriginalDataPk,
      "fieldName": fieldName
    },
    ...
    success: function success(data) {
      ...
      for (var i in data.possibleList) {
        if (i == 0) {
          libraryFunction = fnGetPreprocessFunctionParameters(data.possibleList[i].id);
        }
...
var fnCreatePreprocess = function () {
  ...
  if ($("#preprocess_beginner").hasClass("active")) {
    paramData = {
      "beginner": true,
      "project_id": projectSequencePk,
      "original_data_id": selectedOriginalDataPk,
