...
$(function () {
  ...
  $(document).on("click", ".preprocessedData", function () {
    ...
    selectedPreprocessedName = $(this)[0].innerText;
...
var fnGetPreprocessedDataList = function () {
  ...
  if (fnNotNullAndEmpty(selectedOriginalDataPk)) {
    var preprocessedDataList = fnGetPreprocessedDataListByAjax(selectedOriginalDataPk);
    /*전처리 리스트 가져오기*/
    ...
    for (var i in preprocessedDataList) {
      ...
      if (i == 0) {
        ...
        selectedPreprocessedName = pData.name;
        selectedPreprocessedDataPk = pData.id;
...
var fnGetPreprocessedData = function (pData) {
  ...
  if (pData == null) {
    pData = fnGetPreprocessedDataByAjax(selectedOriginalDataPk, selectedPreprocessedDataPk);
...
var fnSetStatistics = function (statistics) {
  ...
  $("#preName").html(selectedPreprocessedName); //전처리차트 전역변수
...
var fnChangePreprocessedState = function () {
  var pData = fnGetPreprocessedDataByAjax(selectedOriginalDataPk, selectedPreprocessedDataPk);
...
var fnDeletePreprocessedData = function () {
  if (confirm("전처리 데이터를 삭제하면 생성된 모델이 같이 삭제됩니다. \n삭제하시겠습니까?")) {
    var response = fnDeletePreprocessedDataByAjax(projectSequencePk, selectedPreprocessedDataPk);
...
var setTrainParams = function () {
  ...
  var sampleData = JSON.parse(pData.sample_data.value);
  ...
  $("#xParameters").html(xParams);
  $("#yParameters").html(yParams);
...
var fnSetAlgorithmParamsForBeginner = function (algorithm) {
  ...
  var tParamY = "<div class=\"form-check\">";

  for (var _i in statistics) {
    tParamY += "<span class='floatL'>";
    tParamY += "	<input type=\"radio\" class=\"flat\" name=\"yParamCheckbox\"  id=\"yParamCheckbox" + _i + "\" value=\"" + statistics[_i].name + "\">";
    tParamY += "	<label class=\"form-check-label pdright_10\" for=\"yParamCheckbox" + _i + "\">" + statistics[_i].name + "</label>";
    tParamY += "</span>";
  }

  tParamY += "</div>";
  var tParamX = "	<div class=\"form-check form-check-inline\">";
  tParamX += "<p>";
  tParamX += "	<input type=\"checkbox\" class=\"flat\" name=\"xParamAll\" id='check_all'>";
  tParamX += "	<label class=\"form-check-label pdright_10 pdLeft_5 fontWeight_bold\" for=\"xParamAll\">전체 선택</label>";
  tParamX += "</p>";

  for (var _i2 in statistics) {
    tParamX += "<span class='floatL'>";
    tParamX += "	<input class=\"form-check-input\" type=\"checkbox\" name=\"xParamCheckbox\" id=\"xParamCheckbox" + _i2 + "\" value=\"" + statistics[_i2].name + "\">";
    tParamX += "	<label class=\"form-check-label pdright_10 pdLeft_5\" for=\"xParamCheckbox" + _i2 + "\">" + statistics[_i2].name + "</label>";
    tParamX += "</span>";
  }
  ...
  for (var _i3 in trainParams) {
    ...
    if ("y" == tp.name) tParamHtml += "	<td>" + tParamY + "</td>"; else if ("X" == tp.name) tParamHtml += "	<td>" + tParamX + "</td>"; else tParamHtml += "	<td><input type='text' class='form-control' onkeyup='fnRemoveChar(event);' placeholder='" + tp.default + "'></td>";
    tParamHtml += "</tr>";
  }
...
var fnCreateModel = function () {
  ...
  if (fnNotNullAndEmpty(algorithmsSequencePk)) {
    data["project_id"] = projectSequencePk;
    data["algorithm_id"] = algorithmsSequencePk;
    trainData["preprocessed_data_id"] = selectedPreprocessedDataPk;
