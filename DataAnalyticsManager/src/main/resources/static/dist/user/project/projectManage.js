...
$(function () {
  ...
  $(document).on("click", "#tbodyHtml td", function () {
    var index = 4;
    if ("admin" == userAuth) index = 5;

    if ($(this).index() == index) {
      var clickRows = $("#datatable-checkbox").dataTable().fnGetPosition(this); // 변경하고자 하는 clickRow
...
var projectTemplate = function (data) {
  ...
  if (userAuth == "admin") html += "	<td>" + data.user_id + "</td>";
...
var fnUpdateTable = function (data, option) {
  ...
  if (option == "regist") {
    ...
    if (userAuth == "admin") {
      $("#datatable-checkbox").dataTable().fnUpdate([checkbox, data.id, data.name, data.description, data.create_date, data.user_id, modifyBtn], clickRow);
    } else {
      $("#datatable-checkbox").dataTable().fnUpdate([checkbox, data.id, data.name, data.description, data.create_date, modifyBtn], clickRow);
    }
  }
};
